...
def load_mtl(fn, clear_ks=True):
    ...
    for line in lines:
        ...
        if 'newmtl' in prefix:
            ...
            if 'bsdf' in prefix or 'map_kd' in prefix or 'map_ks' in prefix or 'bump' in prefix or'map_d' in prefix or'map_ka' in prefix:
    ...
    for mat in materials:
        ...
        if 'map_kd' in mat:
            ...
            # if(mat['kd'].data.shape[3]==3):
            #     a = torch.ones([3, 3, 3, 64])
        ...
        for tex in mat:
            if(tex=='kd'and mat[tex].data.shape[3]==3):
                m=torch.ones(mat[tex].data.shape[0],mat[tex].data.shape[1],mat[tex].data.shape[2],1, device='cuda')
                n=mat[tex].data
                mat[tex].data=torch.cat((n,m),dim=3)
                print(mat[tex].data.shape)
...
def merge_materials(materials, texcoords, tfaces, mfaces):
    ...
    for tex in textures:
        if tex in materials[0]:
            # tex_data = torch.cat(tuple(util.scale_img_nhwc(mat[tex].data[:,:,:,0:3], tuple(max_res)) for mat in materials), dim=2) # Lay out all textures horizontally, NHWC so dim2 is x
            tex_data = torch.cat(tuple(util.scale_img_nhwc(mat[tex].data, tuple(max_res)) for mat in materials), dim=2)
